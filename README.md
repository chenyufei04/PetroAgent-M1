...
- “为什么推荐当前方案？”：在聊天区列出当前实验全部候选方案按钮；点击任意方案后，
  基于其排名、技术/经济状态、净增量价值、增量油、成本和参数组合解释推荐或不推荐原因；
- “只看未通过规则和证据”：直接列出失败规则、实际值、阈值和单位，同时聚焦规则与证据链；
- “展示关键指标和图谱”：直接汇总当前方案的排名、增量油、净值、成本和参数，并同步显示解释图谱；
- “查看全部方案排名”：切换到方案排名表，并返回方案总数、当前名次及前三名确定性摘要。
...
排名交互与页面排名表读取同一份确定性数据。“查看全部方案排名”会返回方案总数、当前
方案名次和前三名摘要；“为什么第一名优于第二名”会同时比较两者的净增量价值、增量油、
聚合物成本和技术可行性，并引用当前排序政策。此类可由表格直接计算的问题不交给模型
自行补数，避免对话结论与页面表格不一致；开放式解释仍由 Ollama/Qwen 基于完整排名上下文组织。

页面采用助手优先的渐进式布局。首次进入只显示平台标题、居中的交互式工程分析助手，
以及底部折叠的“数据导入、通用规则分析与完整知识库管理”入口；实验方案卡片、指标、
规则、图谱和图表不会在首屏一次性展开。用户发送展示或分析指令后，页面按助手返回的
受控区域自动展开实验工作台；需要恢复简洁界面时可点击“收起分析区”。
